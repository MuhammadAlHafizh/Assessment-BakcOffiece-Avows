...
@Component({
  ...
  template: `
    <div class="flex items-center gap-1.5 select-none">
      <span class="text-xs font-medium text-slate-400">Page</span>
      ...
      <select [ngModel]="page" (ngModelChange)="changePage($event)"
        ...
        <option *ngFor="let p of pagesArray" [value]="p" class="bg-slate-900 text-white dark:bg-slate-900">{{ p }}</option>
      </select>
